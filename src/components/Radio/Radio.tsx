...
const Radio = (props: RadioProps) => {
    ...
    const radioClasses = ReplaceSpaces(`${color} ${size} ${disabled} ${props.classes}`)
    ...
    const RadioBtn = () => {
        return (
            <input
                type="radio"
                name={props.name}
                className={`radio ${radioClasses} checked:${props.checkedColor ? `bg-${props.checkedColor} text-${props.checkedColor}-content` : 'bg-red-500'}`}
