...
export const Menu = (props: MenuProps) => {
    const classes = ReplaceSpaces(
        ...
        } ${props.padding ? `p-${props.padding}` : ''} ${props.compact ? 'menu-compact' : ''} ${props.horizontal ? 'menu-horizontal' : ''}`
    )
    return (
                ...
                <>
                    ...
                    {menu.items?.map((item, i) => (
                        <li key={item.id || item.url || item.value || 'Item' + i}>
                            <a
                                href={item.url}
                                className={`${item.isActive ? 'active' : ''} ${item.bordered ? 'bordered' : ''} ${props.borderHover ? 'hover-bordered' : ''} ${menu.disabled ? 'disabled' : ''}`}
                            >
                                {item.icon}
