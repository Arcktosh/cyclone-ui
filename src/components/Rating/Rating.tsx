...
export const Rating = (props: RatingProps) => {
    ...
    return (
        <div className={classes} style={props.style}>
            <input type="radio" className="rating-hidden" onClick={() => SetRating(0)} />
            {props.half
                ? [...Array(props.count ? props.count * 2 : 5 * 2)].map((item, index) => {
                      const givenRating = index + 1
                      const halfClass = givenRating % 2 === 0 ? ' mask-half-2' : ' mask-half-1'
                      return (
                          <input
                              type="radio"
